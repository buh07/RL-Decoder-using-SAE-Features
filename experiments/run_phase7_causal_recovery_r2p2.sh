...
usage() {
  cat <<'USAGE'
Usage:
  experiments/run_phase7_causal_recovery_r2p2.sh launch
  experiments/run_phase7_causal_recovery_r2p2.sh canary-worker <RUN_ID> <RUN_TAG> <BASE> <GPU>
  experiments/run_phase7_causal_recovery_r2p2.sh promote-worker <RUN_ID> <RUN_TAG> <BASE> <GPU> <GROUP>
  experiments/run_phase7_causal_recovery_r2p2.sh coordinator <RUN_ID> <RUN_TAG> <BASE>
USAGE
}
...
PY
      then
        run_chain_for_var "$run_tag" "$ckpt_id" "$ckpt" "$mode_tag" "$var" "$device" 2200 >/dev/null
...
need = (not math.isfinite(cov) or cov < 0.25) and (not math.isfinite(cf) or cf < 0.20)
sys.exit(0 if need else 1)
PY
        then
          run_chain_for_var "$run_tag" "$ckpt_id" "$ckpt" "$mode_tag" "$var" "$device" 3200 >/dev/null
        fi
      fi
    done
  done
...
pass_cov=(len(cov)>0 and min(cov) >= 0.25)
...
out={
  ...
  "criteria":{
    ...
    "causal_signal_coverage_fraction_min":0.25,
...
PY
    ...
    if [[ "$canary_pass" == "true" ]]; then
      tmux new-session -d -s "p7r22_promote_g6_${run_id}" "cd '$PWD' && bash experiments/run_phase7_causal_recovery_r2p2.sh promote-worker '$run_id' '$run_tag' '$base' 6 raw"
      tmux new-session -d -s "p7r22_promote_g7_${run_id}" "cd '$PWD' && bash experiments/run_phase7_causal_recovery_r2p2.sh promote-worker '$run_id' '$run_tag' '$base' 7 hybrid"
...
launch() {
  local run_id
  run_id="$(date +%Y%m%d_%H%M%S)_phase7_causal_recovery_r2p2"
  local run_tag="phase7_causal_recovery_r2p2_${run_id}"
...
refs={
  "schema_version":"phase7_r2p2_baseline_refs_v1",
  "run_tag":run_tag,
...
contract={
  "schema_version":"phase7_r2p2_closure_contract_v1",
  "run_tag":run_tag,
...
PY
  ...
  tmux new-session -d -s "p7r22_canary_g6_${run_id}" "cd '$PWD' && bash experiments/run_phase7_causal_recovery_r2p2.sh canary-worker '$run_id' '$run_tag' '$base' 6"
  tmux new-session -d -s "p7r22_coord_${run_id}" "cd '$PWD' && bash experiments/run_phase7_causal_recovery_r2p2.sh coordinator '$run_id' '$run_tag' '$base'"

  echo "RUN_ID=$run_id"
  echo "RUN_TAG=$run_tag"
  echo "BASE=$base"
  echo "Sessions:"
  echo "  p7r22_canary_g6_${run_id}"
  echo "  p7r22_coord_${run_id}"
}
